    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                <h2><i class="fa fa-list-alt bg-orange"></i> Add Article <small
                            ...
                            <table class="table table-striped table-dark table-bordered">
                                ...
                                @foreach($articles as $article)
                                    <tr>
                                        ...
                                        <td>{{ $article->tag_1 }} &nbsp; {{ $article->tag_2 }}
                                            &nbsp; {{ $article->tag_3 }}&nbsp; {{ $article->tag_4 }}</td>
                                        <td>{{ $article->type }}</td>
                                        <td><img src="../storage/app/{{ $article->image }}" width="100px"
                                                 height="100px"></td>
                                        <td>{{ $article->content }}
                                        <td>
                                            <a href="{{ route('editArticle', $article->id) }}"><i
                                                    class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;
                                            <a href="#">
                                                <form method="POST" id="delete-{{ $article->id }}"
                                                      action="{{ route('deleteArticle', $article->id) }}"
                                                      style="display-none;">
                                                    @csrf
                                                    {{ method_field('delete') }}
                                                </form>
